...
window.GeocacheSolverController = class extends Stimulus.Controller {
    ...
    connect() {
        ...
        document.addEventListener('coordinatesDetected', function(event) {
            console.log("ÃvÃ©nement coordinatesDetected reÃ§u directement par le document:", event.detail);
        });
    }
    ...
    handleCoordinatesDetected(event) {
        ...
        if (event.detail && event.detail.exist) {
            console.log("CoordonnÃ©es valides dÃ©tectÃ©es, stockage uniquement");
            
            // Stocker les coordonnÃ©es dÃ©tectÃ©es sans mise Ã jour de l'affichage
    ...
    // AJOUT: MÃ©thode simplifiée pour displayDetectedCoordinates (garde l'interface compatible)
    ...
    async loadPluginsList() {
        try {
            ...
            console.log("Ãtat du panneau avant:", this.pluginsPanelTarget.classList.contains('hidden') ? 
"cachÃ©" : "visible");
            this.pluginsPanelTarget.classList.remove('hidden');
            console.log("Ãtat du panneau aprÃ¨s:", this.pluginsPanelTarget.classList.contains('hidden') ? 
"cachÃ©" : "visible");
            ...
            let pluginsListHtml = `
                <div id="${mainPluginZoneId}" class="bg-gray-800 rounded-lg p-4" 
data-plugin-selection-zone="true">
                    ...
                    <div class="mb-4">
                        <input 
                            ...
                            class="w-full px-3 py-2 bg-gray-700 text-gray-200 rounded-md border 
border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
            ...
            plugins.forEach(plugin => {
                pluginsListHtml += `
                    <div class="plugin-item flex items-center justify-between p-2 rounded-md 
hover:bg-gray-700 transition-colors" data-plugin-name="${plugin.name.toLowerCase()}">
                        <div>
                            <div class="font-medium text-gray-200">${plugin.name}</div>
                            <div class="text-xs text-gray-400">${plugin.description || 'Aucune 
description'}</div>
                        </div>
                        <button 
                            ...
                            class="px-3 py-1 bg-purple-600 hover:bg-purple-700 text-white rounded-md 
...
pluginZone.querySelector('[data-action="click->geocache-solver#executePlugin"]');
                    if (executeButton) {
                        ...
                        console.log("Attribut data-plugin-zone-id ajoutÃ au bouton d'exÃ©cution:", 
pluginZoneId);
                    }
                    ...
                    // Dans ce cas, nous sommes dans le panneau principal, crÃons un ID
...
principal:", mainPanelId);
                ...
                // PrÃ©parer le texte Ã traiter (soit le rÃ©sultat du plugin prÃ©cÃ©dent, soit le texte de 
la description)
    ...
    async executePlugin(event) {
        ...
        if (pluginId && pluginName) {
            ...
            console.log("Valeur dans _currentDescriptionValue:", this._currentDescriptionValue || "non 
dÃ©finie");
...
document.getElementById(button.dataset.pluginZoneId) : null;
            ...
            if (pluginInputTextarea && pluginInputTextarea.value.trim() !== '') {
                ...
                if (textToProcess.trim() === '' && this.lastPluginOutputValue) {
                    ...
                    console.log("Description vide, utilisation du rÃ©sultat du plugin prÃ©cÃ©dent:", 
textToProcess);
                }
            }
...
document.getElementById(`result-for-${button.dataset.pluginZoneId}`);
                    ...
                    resultContainer.innerHTML = `
                        <h2 class="text-lg font-semibold text-gray-100 mb-3">RÃ©sultat du plugin 
${pluginName}</h2>
                        <div id="${resultZoneId}-text" class="w-full min-h-[100px] bg-gray-700 
text-gray-200 p-3 rounded-lg border border-gray-600 focus:border-blue-500 focus:ring-2 focus:ring-blue-500 
space-y-3 overflow-auto">
                ...
                if (resultTextContainer) {
                    if (resultTextContainer.tagName === 'TEXTAREA') {
                        ...
                        resultTextContainer.innerHTML = '<div class="animate-pulse"><div class="h-4 
bg-gray-700 rounded w-3/4 mb-2"></div><div class="h-4 bg-gray-700 rounded w-1/2"></div></div>';
                    }
                }
                ...
                if (pluginZone) {
                    ...
                    inputs.forEach(input => {
                        ...
                        if (inputName === 'text') {
                            console.log("Champ 'text' trouvÃ© dans le formulaire, ignorÃ© pour ne pas 
Ã©craser l'entrÃ©e");
                            return;
                        }
                ...
                if (pluginInputs.text !== textToProcess) {
                    console.error("ANOMALIE: Le texte dans les paramÃ¨tres a Ã©tÃ© modifiÃ©, 
correction...");
                    pluginInputs.text = textToProcess;
                }
                        ...
                        resultHtml += `
                            <div class="bg-gray-700 rounded-lg p-4 mb-3">
                                ...
                                <div class="bg-gray-800 p-3 rounded">
                                    <p class="text-gray-300">${data.summary.message || 
`${data.summary.total_results} rÃ©sultat(s)`}</p>
                                    ${data.plugin_info ? `<p class="text-gray-400 text-xs mt-1">Temps 
d'exÃ©cution: ${data.plugin_info.execution_time || 0} ms</p>` : ''}
                                </div>
                            </div>`;
                    }
                    ...
                    data.results.forEach((result, index) => {
                        ...
                        resultHtml += `
                            <div class="bg-gray-700 rounded-lg p-3 mb-3 ${isBest ? 'border 
border-blue-500' : ''}">
                                <div class="flex justify-between items-start mb-2">
                                    <h4 class="text-md font-medium ${isBest ? 'text-blue-400' : 
'text-gray-300'}">
                                        RÃ©sultat ${index + 1} ${isBest ? '(Meilleur rÃ©sultat)' : ''}
                                    </h4>
                                    <div class="bg-gray-900 px-2 py-1 rounded text-xs">
                                        Confiance: <span class="font-bold 
${confidenceColor}">${Math.round((result.confidence || 0) * 100)}%</span>
                                    </div>
                                </div>
                                
                                <div class="bg-gray-800 p-3 rounded mb-2">
                                    <div class="text-sm text-gray-300 
whitespace-pre-wrap">${result.text_output || ''}</div>
                                </div>`;
                        ...
                        if (result.parameters && Object.keys(result.parameters).length > 0) {
                            resultHtml += `
                                <div class="mt-2 p-2 bg-gray-800 rounded">
                                    <h5 class="text-xs font-medium text-gray-400 mb-1">ParamÃ¨tres 
utilisÃ©s:</h5>
                                    <div class="text-xs text-gray-300 grid grid-cols-2 gap-1">
                                        ${Object.entries(result.parameters).map(([key, value]) => 
                                            `<div><span class="text-gray-500">${key}:</span> 
${value}</div>`
                                        ).join('')}
                                    </div>
                                </div>`;
                        ...
                        if (result.coordinates && result.coordinates.exist) {
                            ...
                            resultHtml += `
                                <div class="mt-2 p-2 bg-gray-800 rounded">
                                    <h5 class="text-xs font-medium text-gray-400 mb-1">CoordonnÃ©es 
GPS:</h5>
                                    <div class="text-xs text-gray-300">
                                        <div class="mb-1"><span class="text-gray-500">Format DDM:</span> 
${result.coordinates.ddm || ''}</div>
                                        <div class="grid grid-cols-2 gap-1">
                                            <div><span class="text-gray-500">Latitude:</span> 
${result.coordinates.ddm_lat || ''}</div>
                                            <div><span class="text-gray-500">Longitude:</span> 
${result.coordinates.ddm_lon || ''}</div>
                                        </div>
                                    </div>
                                </div>`;
                        }
...
JSON.stringify(data.result, null, 2);
                    ...
                    if (coordinates) {
                        resultHtml += `
                            <div class="bg-gray-900 rounded-lg p-3 border border-gray-700 mb-3">
                                <h4 class="text-md font-medium text-green-400 mb-2">CoordonnÃ©es GPS 
dÃ©tectÃ©es</h4>
                                <div class="text-sm text-gray-300">
                ...
                if (resultTextContainer) {
                    ...
                    // Ajouter un Ã©couteur d'Ã©vÃ©nement pour mettre Ã jour lastPluginOutputValue lorsque 
le texte est modifiÃ©
                    if (resultTextContainer.tagName === 'TEXTAREA') {
                        resultTextContainer.addEventListener('input', (e) => {
                            this.lastPluginOutputValue = e.target.value;
                            console.log("Valeur mise Ã jour aprÃ¨s modification:", 
this.lastPluginOutputValue);
                ...
                // Stocker le rÃ©sultat pour le prochain plugin (uniquement si ce n'est pas une 
rÃ©exÃ©cution)
                if (!isReexecution) {
                    this.lastPluginOutputValue = resultText;
                }
                
                // NOUVEAU: DÃ©tecter le format spÃ©cifique avec espaces entre chiffres pour les 
coordonnÃ©es GPS
                if (!coordinates) {
                    // Expression rÃ©guliÃ¨re pour dÃ©tecter le format "N 4 9 1 2 1 2 3 E 0 0 6 1 2 1 2 3"
                    const spacedFormatRegex = /([NS])\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+([
EW])\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)/i;
                    ...
                    if (match) {
                        ...
                        const latDigits = match[2] + match[3] + match[4] + match[5] + match[6] + match[7] 
+ match[8];
                        const lonDir = match[9];
                        const lonDigits = match[10] + match[11] + match[12] + match[13] + match[14] + 
match[15] + match[16];
                        ...
                        try {
                            ...
                            // Pour la longitude, prÃ©fixer avec des 0 pour avoir 3 chiffres pour les 
degrÃ©s
                            ...
                            const convertedCoords = {
                                ...
                                ddm: ddm,
                                decimal: {
                                    latitude: parseFloat(latDir === 'N' ? 1 : -1) * (parseFloat(latDeg) + 
parseFloat(`${latMin}.${latDec}`) / 60),
                                    longitude: parseFloat(lonDir === 'E' ? 1 : -1) * (parseFloat(lonDeg) + 
parseFloat(`${lonMin}.${lonDec}`) / 60)
                                },
                                patterns: [match[0]]
                            };
                            
                            // Au lieu d'afficher les coordonnÃ©es sÃ©parÃ©ment, les inclure directement 
dans le HTML du rÃ©sultat
                            if (resultTextContainer && resultTextContainer.tagName !== 'TEXTAREA') {
                                // Pour les conteneurs HTML, ajouter la section de coordonnÃ©es au HTML 
existant
                                const coordsHtml = `
                                    <div class="mt-3 p-3 bg-gray-800 rounded">
                                        <div class="flex items-center justify-between mb-2">
                                            <h4 class="text-sm font-medium text-blue-400">CoordonnÃ©es GPS 
dÃ©tectÃ©es</h4>
                                            <span class="px-3 py-1 text-xs rounded-full 
bg-green-600">Format avec espaces</span>
                                        </div>
...
w-20">Longitude:</span>
                                            ...
                                            <div class="mt-2 flex gap-2">
                                                <button type="button" 
                                                        class="text-xs bg-blue-700 hover:bg-blue-600 
text-white px-3 py-1 rounded cursor-pointer copy-coords" 
                                                        data-coords="${ddm}"
                                                        onclick="navigator.clipboard.writeText('${ddm}')">
                                                    Copier
                                                </button>
                                                <button type="button" 
                                                        class="text-xs bg-purple-700 hover:bg-purple-600 
text-white px-3 py-1 rounded cursor-pointer create-waypoint"
                                                        data-ddm="${ddm}" 
                                                        data-lat="${ddmLat}" 
                                                        data-lon="${ddmLon}"
...
data-action="click->geocache-solver#useCoordinates">
                            ...
                            // Ã‰mettre Ã©galement l'Ã©vÃ©nement pour que d'autres composants puissent 
rÃ©agir
                            ...
                            console.error("Erreur lors de la conversion des coordonnÃ©es espacÃ©es:", 
error);
                        }
                    }
                }
                ...
                buttonContainer.innerHTML = `
                    <button 
                        ...
                        class="flex items-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white 
                ...
                if (resultTextContainer) {
                    if (resultTextContainer.tagName === 'TEXTAREA') {
                        ...
                        resultTextContainer.innerHTML = `
                            <div class="bg-red-900/50 border border-red-500 text-red-200 px-4 py-3 
rounded">
...
text-xs font-medium transition-colors">
    ...
    async executeMetaSolver(event) {
        ...
        try {
            ...
            console.log("Nombre d'Ã©lÃ©ments dans 'results':", result.results && 
Array.isArray(result.results) ? result.results.length : 0);
    ...
    async decodeWithPlugin(event) {
        ...
        try {
            ...
            console.log("decodeWithPlugin - Type de 'results':", result.results ? typeof result.results : 
"non dÃ©fini");
            console.log("decodeWithPlugin - Nombre d'Ã©lÃ©ments dans 'results':", result.results && 
Array.isArray(result.results) ? result.results.length : 0);
    ...
    async formatMetaDetectionResults(result) {
        ...
        if (result.primary_coordinates) {
            console.log("formatMetaDetectionResults - CoordonnÃ©es primaires dÃ©tectÃ©es:", 
result.primary_coordinates);
            ...
            if (!ddmFull && result.primary_coordinates) {
                ...
                ddmFull = `${lat > 0 ? 'N' : 'S'} ${Math.abs(lat).toFixed(6)}Â°, ${lon > 0 ? 'E' : 'W'} 
${Math.abs(lon).toFixed(6)}Â°`;
            }
            ...
            console.log("Ã‰mission de l'Ã©vÃ©nement coordinatesDetected depuis MetaSolver avec les 
coordonnÃ©es:", standardizedCoords);
            ...
            gpsCoordinatesHtml = `
                <div class="bg-gray-700 rounded-lg p-4 mt-4">
                    ...
                    <div class="bg-gray-800 p-4 rounded grid grid-cols-1 gap-4">
                        <div>
                            <label for="coords_ddm" class="block text-sm font-medium text-gray-400 
mb-1">CoordonnÃ©es:</label>
                            <input type="text" id="coords_ddm" class="w-full bg-gray-800 text-green-300 
border border-gray-700 focus:border-green-500 p-2 rounded font-mono"
                                   value="${ddmFull}" readonly>
                        </div>
                        
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="coords_lat" class="block text-sm font-medium text-gray-400 
mb-1">Latitude:</label>
                                <input type="text" id="coords_lat" class="w-full bg-gray-800 
text-green-300 border border-gray-700 focus:border-green-500 p-2 rounded font-mono"
                                       value="${ddmLat || result.primary_coordinates.latitude}" readonly>
                            </div>
                            <div>
                                <label for="coords_lon" class="block text-sm font-medium text-gray-400 
mb-1">Longitude:</label>
                                <input type="text" id="coords_lon" class="w-full bg-gray-800 
text-green-300 border border-gray-700 focus:border-green-500 p-2 rounded font-mono"
                        ...
                        <div class="mt-2">
                            <a href="https://www.google.com/maps?q=${result.primary_coordinates.latitude},$
{result.primary_coordinates.longitude}" 
                               target="_blank" 
...
inline-flex items-center mr-2">
                            ...
                            <button class="px-3 py-1 text-sm bg-green-600 hover:bg-green-700 text-white 
rounded-md"
                                    onclick="navigator.clipboard.writeText('${ddmFull}').then(() => 
alert('CoordonnÃ©es copiÃ©es dans le presse-papier'))">
            ...
            console.log("formatMetaDetectionResults - CoordonnÃ©es anciennes dÃ©tectÃ©es (format 
rÃ©trocompatible)");
...
text-green-300 border border-gray-700 focus:border-green-500 p-2 rounded font-mono"
                        ...
                        <div class="flex justify-between mt-2">
                            <button class="px-3 py-1 text-sm bg-blue-600 hover:bg-blue-700 text-white 
rounded-md"
                                    data-action="click->geocache-solver#useCoordinates">
...
onclick="navigator.clipboard.writeText('${result.coordinates.ddm}').then(() => alert('CoordonnÃ©es 
copiÃ©es dans le presse-papier'))">
                                Copier
                            </button>
                        </div>
                    </div>
        ...
        if (result.results && Array.isArray(result.results) && result.results.length > 0) {
            console.log("formatMetaDetectionResults - DÃ©tectÃ© format standardisÃ© avec tableau results", 
result.results);
            let html = '';
            ...
            if (result.summary) {
                ...
                html += `
                    <div class="bg-gray-700 rounded-lg p-4 mb-3">
                        <h3 class="text-md font-medium text-blue-400 mb-2">RÃ©sumÃ©</h3>
                        <div class="bg-gray-800 p-3 rounded">
                            <p class="text-gray-300">${result.summary.message || 
`${result.summary.total_results} rÃ©sultat(s)`}</p>
                            ${result.plugin_info ? `<p class="text-gray-400 text-xs mt-1">Temps 
d'exÃ©cution: ${result.plugin_info.execution_time || 0} ms</p>` : ''}
                        </div>
                    </div>`;
            }
            ...
            result.results.forEach((resultEntry, index) => {
                ...
                console.log(`RÃ©sultat ${index} - Plugin: ${parameterPlugin}, Mode: 
${resultEntry.parameters?.mode}, CanDecode: ${canDecode}, ShowButton: ${showDecodeButton}`);
                    ...
                    <div class="bg-gray-600 p-3 rounded ${isBest ? 'border border-blue-500' : ''}">
                        <div class="flex justify-between items-center mb-2">
                            <div class="flex items-center">
                                ...
                                <div class="text-xs ${confidenceColor} ml-2">Confiance: 
${Math.round((resultEntry.confidence || 0) * 100)}%</div>
                            </div>
                            ${showDecodeButton ? 
                                `<button 
                                    data-action="click->geocache-solver#decodeWithPlugin" 
                                    data-plugin="${parameterPlugin}" 
                                    class="bg-blue-600 hover:bg-blue-700 text-white text-sm py-1 px-3 
rounded">
                                    DÃ©coder
                                </button>` : ''}
                        </div>
                        <div class="text-sm text-gray-300 whitespace-pre-wrap">${resultEntry.text_output 
|| ''}</div>
            ...
            html += `</div></div>`;
            console.log("formatMetaDetectionResults - HTML gÃ©nÃ©rÃ© pour le format standardisÃ© 
(longueur):", html.length);
        ...
        // Afficher les rÃ©sultats des plugins combinÃ©s (nouveau format)
            ...
            console.log("formatMetaDetectionResults - Utilisation du format combined_results", 
Object.keys(result.combined_results));
            let html = `
                <div class="bg-gray-700 p-4 rounded-lg">
                    <h3 class="text-lg font-semibold text-blue-400 mb-3">RÃ©sultats de l'analyse</h3>
                    <div class="space-y-3">
            ...
            Object.entries(result.combined_results).forEach(([pluginName, pluginResult]) => {
                ...
                if (pluginResult.confidence !== undefined) {
                    ...
                    confidenceText = `<div class="text-xs ${confidenceColor} font-medium">Confiance: 
${Math.round(confidence * 100)}%</div>`;
            ...
            html += `</div></div>`;
            console.log("formatMetaDetectionResults - HTML gÃ©nÃ©rÃ© pour le format combined_results 
(longueur):", html.length);
            return html + gpsCoordinatesHtml;
        }
...
{
            console.log("formatMetaDetectionResults - Utilisation du format possible_codes ancien", 
result.result.possible_codes);
            ...
            for (const code of codes) {
                ...
                if (code.confidence !== undefined) {
                    ...
                    const confidenceColor = this.getConfidenceColor(confidence);
                    confidenceText = `<div class="text-xs ${confidenceColor} ml-2">Confiance: 
${Math.round(confidence * 100)}%</div>`;
                }
                ...
                html += `
                    <div class="bg-gray-600 p-3 rounded">
                        ...
                        <div class="text-sm text-gray-400 mt-1">${fragments.length} fragment(s): 
${fragmentsText}</div>
                    </div>
                `;
            }
            ...
            html += `</div></div>`;
            console.log("formatMetaDetectionResults - HTML gÃ©nÃ©rÃ© pour le format possible_codes 
(longueur):", html.length);
            return html + gpsCoordinatesHtml;
        }
        // GÃ©rer l'ancien format (dÃ©codage spÃ©cifique)
        else if (result.result && result.result.decoded_results && result.result.decoded_results.length > 
0) {
            console.log("formatMetaDetectionResults - Utilisation du format decoded_results ancien", 
result.result.decoded_results);
...
${Math.round(confidence * 100)}%</div>`;
            ...
            html += `</div></div>`;
            console.log("formatMetaDetectionResults - HTML gÃ©nÃ©rÃ© pour le format decoded_results 
(longueur):", html.length);
            return html + gpsCoordinatesHtml;
        } 
        // GÃ©rer l'ancien format (dÃ©codage simple)
        else if (result.result && result.result.decoded_text) {
            console.log("formatMetaDetectionResults - Utilisation du format decoded_text ancien", 
result.result.decoded_text);
            ...
            console.log("formatMetaDetectionResults - Aucun format reconnu, affichage du message par 
dÃ©faut", result);
            return `
                <div class="bg-gray-700 p-4 rounded-lg">
    ...
    useCoordinates(event) {
        ...
        if (this.geocacheIdValue) {
            if (confirm("Voulez-vous utiliser ces coordonnÃ©es comme coordonnÃ©es corrigÃ©es pour cette 
gÃ©ocache?")) {
    ...
    async saveCoordinatesToGeocache(coords) {
        try {
            ...
            if (response.ok) {
                ...
                alert(`Erreur lors de l'enregistrement des coordonnÃ©es: ${error.message || 'Erreur 
inconnue'}`);
            }
        } catch (error) {
...
inconnue'}`);
    ...
    async useCoordinates() {
        ...
        if (!this.lastDetectedCoordinatesValue || !this.geocacheIdValue) {
            console.error("Impossible d'utiliser les coordonnÃ©es: coordonnÃ©es non dÃ©tectÃ©es ou ID de 
gÃ©ocache non dÃ©fini");
            return;
        }
...
inconnue'}`);
    ...
    resetCoordinatesDisplay() {
        console.log("RÃ©initialisation des coordonnÃ©es dÃ©tectÃ©es");
